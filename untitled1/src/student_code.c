...
void pass_to_client_handler(void* arg) {
  // todo: Convert this to run multi-threaded
    ...
    pthread_t new_thread; // create a new thread
    pthread_create(&new_thread, NULL, &client_handler, arg); // run the client handler in the new thread
}
...
void read_lock(PlayerDatabase* db) {
  ...
  // We need to check the number of current readers and writers

    pthread_mutex_lock(db->mutex); // lock the mutex
    while (db->writers > 0 || db->readers >= MAX_CONCURRENT_READERS) {
        pthread_cond_wait(db->can_read, db->mutex); // wait for the condition to be met
    }
    db->readers++; // increment the number of readers
...
void read_unlock(PlayerDatabase* db) {
  // todo
    ...
    pthread_mutex_lock(db->mutex); // lock the mutex
    db->readers--; // decrement the number of readers
    if (db->readers == 0) {
        pthread_cond_signal(db->can_write); // signal that we can write
    }
    pthread_mutex_unlock(db->mutex); // unlock the mutex
}
...
void write_lock(PlayerDatabase* db) {
  // todo
    ...
    while (db->readers > 0 || db->writers > 0) {
        pthread_cond_wait(db->can_write, db->mutex); // wait for the condition to be met
    }
    db->writers++; // increment the number of writers
    pthread_mutex_unlock(db->mutex); // unlock the mutex
}
...
void write_unlock(PlayerDatabase* db) {
  // todo
    ...
    pthread_mutex_lock(db->mutex); // lock the mutex
    db->writers--; // decrement the number of writers
    if (db->writers == 0 && db->readers == 0) { // if there are no writers or readers
...
int add_player_score(PlayerDatabase* db, char* player_name, int score) {
  ...
  // No, we only need to lock the specific player's stats
  write_lock(db);
